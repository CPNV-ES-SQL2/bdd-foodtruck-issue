...
### De la Normalisation
- Organiser les données de manière a minimiser la redondance.
- Permettre une meilleure maintenabilité des bases de données.
- Éviter les anomalies lors des opérations de modification (insertion, mise à jour, suppression).
...
Une relation est en BCNF est une forme normale stricte de la 3NF. Une relation est en BCNF si, pour chaque dépendance fonctionnelle X -> Y, X est une superclé. Cela signifie que chaque déterminant doit être une clé candidate.
